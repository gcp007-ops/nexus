...
| **Web Tools** *(experimental, desktop only)* | open, capture-pdf, capture-png, capture-markdown, links | Open any webpage in a headless browser and capture it as a PDF, PNG, or clean Markdown (boilerplate stripped). Also extracts all links with their text and type. Requires desktop; not available on mobile. |
