...
- **#309 — `search content`: casamento difuso de nome supera acerto de conteúdo, e o resultado não diz qual é qual.**
  Branch `fix/search-content-fuzzy-outranks-content`, commit `c4f24fdc`, base `origin/main` `fb7c74b7`.
  Causa-raiz em `searchContent.ts`, `searchInFile`: `fuzzyScore = clamp(1 + fuzzyResult.score / 100)` põe um casamento difuso fraco de **nome** em ~0,95, acima do teto 0,9 que `performKeywordSearch` dá à frase exata no corpo — as duas escalas nunca foram comensuráveis. Correção: `matchType: 'content' | 'path'` por resultado, camada interna de ordenação pondo conteúdo estritamente acima de nome, e o campo documentado no `getResultSchema`. 32 linhas num arquivo; a normalização difusa ficou intocada de propósito.
  Cobertura: 4 casos em `tests/unit/SearchContentTool.test.ts` (arquivo novo), três deles falhando no build stock e o quarto travando a não-regressão do **vazio** — único sinal honesto de "não achei" do tool. Suíte 4112 pass / 36 skip / 4150; as 2 falhas em `DataAnalysisGuards` e `RunPythonTool` reproduzem na base limpa e são alheias.
  Verificação ao vivo: build da tag `5.16.1` + o mesmo commit (branch `fix/search-content-on-5161`, `54867349`, md5 `FEFFF6847ED381F4F2C14577949BC839`) implantado sobre a 5.16.1 instalada; smokes 4/4 com par antes/depois na mesma vault.
