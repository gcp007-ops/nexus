...
export class TaskRepository
  ...
  async update(id: string, data: UpdateTaskData): Promise<void> {
    ...
    try {
      ...
      await this.transaction(async () => {
        ...
        // new histories folds identically — TaskEventApplier stays untouched.
  ...
  private async resolveMetadataInTransaction(
    ...
    if (data.metadataMode !== 'replace') {
      ...
      current = parseJsonColumn<Record<string, unknown>>(row?.metadataJson, `TaskRepository.metadata#${id}`);
    }
