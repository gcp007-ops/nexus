...
describe('envelope guard and the system guides workspace', () => {
  ...
  it('keeps rejecting when the service has no resolver at all', async () => {
    // A plain `workspaceService.getWorkspaceByNameOrId(...)` throws on a
    // service that lacks the member, and that throw lands in the catch that
    // returns null — the whole guard would fail OPEN and accept everything,
    // with no symptom other than rejections quietly stopping. Pins the
    // optional call: a missing member means "cannot resolve", not "accept".
    const listOnly = {
      plugins: {
        getPlugin: (id: string) =>
          id === 'nexus'
            ? { services: { workspaceService: { async listWorkspaces() { return LISTED; } } } }
            : null
      }
    } as unknown as App;

    const service = new ToolBatchExecutionService(listOnly, new Map<string, IAgent>(), []);
